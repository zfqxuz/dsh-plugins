#!/bin/bash
# Install dsh-context-handoff into a DSH home (default ~/.dsh) as a home-level
# local plugin, without pnpm. Idempotent: re-running replaces the managed block.
set -euo pipefail

PLUGIN_ID="context-handoff"
DIR_NAME="dsh-context-handoff"
...
$MARK_BEGIN
# Automatic context-pressure handoff to a new session in the same workspace.
- insert:
    - id: $PLUGIN_ID
